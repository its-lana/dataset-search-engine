...
func Get_All(search string, sort string, filter1 string, page int, tahunAwal int, tahunAkhir int) (int, int, []datastruct.Datasets, error) {
	db := config.CreateConnection()
	...
	if len(search) != 0 {
		...
		if len(filter1) != 0 {
			sqlStatement = fmt.Sprintf("%s AND %s", sqlStatement, filter1)
		}
		if tahunAwal != 0 && tahunAkhir != 0 {
			sqlStatement = fmt.Sprintf("%s AND tahun_awal_data >= %d AND tahun_akhir_data <=%d", sqlStatement, tahunAwal, tahunAkhir)
		}
	} else if len(filter1) != 0 {
		sqlStatement = fmt.Sprintf("%s WHERE %s", sqlStatement, filter1)
		if tahunAwal != 0 && tahunAkhir != 0 {
			sqlStatement = fmt.Sprintf("%s AND tahun_awal_data >= %d AND tahun_akhir_data <=%d", sqlStatement, tahunAwal, tahunAkhir)
		}
	} else {
		if tahunAwal != 0 && tahunAkhir != 0 {
			sqlStatement = fmt.Sprintf("%s WHERE tahun_awal_data >= %d AND tahun_akhir_data <=%d", sqlStatement, tahunAwal, tahunAkhir)
		}
	}
...
func Get_Total(search string, filter1 string, tahunAwal int, tahunAkhir int) (int, int, error) {
	db := config.CreateConnection()
