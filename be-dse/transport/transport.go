...
func Get_All_Datasets(w http.ResponseWriter, r *http.Request) {
	...
	param2 := ""
	if filter != "" {
		param1, param2, err = logging.ValidateAndReturnFilter(filter)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		filterQuery1, err = logging.ValidateAndReturnFilterMap(param1)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		filterQueries = filterQuery1
		if param2 != "" {
			filterQuery2, err = logging.ValidateAndReturnFilterMap(param2)
			filterQueries = fmt.Sprintf("%s AND %s", filterQueries, filterQuery2)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
	...
	totalData, totalPage, datasets, err := service.Get_All(searchQuery, sortQuery, filterQueries, page, int(tahunAwal), int(tahunAkhir))
	if err != nil {
...
func Get_Total_Data(w http.ResponseWriter, r *http.Request) {
	...
	totalData, totalPage, err := service.Get_Total(searchQuery, filterQueries, int(tahunAwal), int(tahunAkhir))
	if err != nil {
		log.Fatalf("Tidak bisa mengambil data. %v", err)
	}
