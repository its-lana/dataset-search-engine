...
func ValidateAndReturnFilter(filter string) (string, string, error) {
	splits := strings.Split(filter, "|")
	param1 := ""
	param2 := ""
	if len(splits) == 2 {
		// return "", "", errors.New("malformed filter query parameter, should be parameter 1|parameter 2")
		param1 = splits[0]
		param2 = splits[1]
...
func ValidateAndReturnFilterMap(filter string) (string, error) {
	...
	fill1 := fmt.Sprintf("%s = '%s'", field, value)
